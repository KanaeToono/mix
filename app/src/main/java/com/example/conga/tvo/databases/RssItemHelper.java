...
public class RssItemHelper extends SQLiteOpenHelper implements ICRUDDatabase {
    ...
    public void addNewItemRss(RssItem rssItem) {
        ...
        try {
            ...
            contentValues.put(KEY_IMAGE, rssItem.getImage());
            database.insert(TABLE_FAVORITES_NEWS, null, contentValues);
        } catch (Exception e) {
            e.printStackTrace();
            Log.e("problem", e + "");
        }
    ...
    public ArrayList<RssItem> getAllItemsRss() {
        ...
        try {
            String QUERY = " SELECT * FROM " + TABLE_FAVORITES_NEWS;
            Cursor cursor = mDatabase.rawQuery(QUERY, null);
            if (cursor.moveToFirst() && cursor.getCount() >= 1) {
                do {
                    ...
                    task.setTitle(cursor.getString(1));
                    task.setLink(cursor.getString(2));
                    task.setPubDate(cursor.getString(3));
                    task.setImage(cursor.getString(4));
                    contentRssArrayList.add(task);
                } while (cursor.moveToNext());
            }
        } catch (Exception e) {
    ...
    public void deleteItemRssItem(int id_rss) {
        SQLiteDatabase mDatabase = this.getWritableDatabase();
        try {
            mDatabase.delete(TABLE_FAVORITES_NEWS, KEY_ID
                    + " =" + id_rss, null);
        } catch (Exception e) {
            e.printStackTrace();
        }
